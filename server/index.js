...
io.on('connection', (socket) => {
  ...
  // add_to_queue — enforces cooldown per socket (using IP address)
  socket.on('add_to_queue', async ({ roomCode, track }) => {
    try {
      if (!track || typeof track !== 'object' || !track.id || !track.name || !track.uri) return;
      const safeTrack = { id: String(track.id), name: String(track.name), artists: track.artists, album: track.album, uri: String(track.uri) };

      const queue = await getQueue(roomCode);
      if (queue.some(t => t.id === safeTrack.id)) {
        socket.emit('request_denied', { reason: 'already_in_queue' });
        return;
      }

      const cooldownSeconds = await getCooldown(roomCode);
      // Use IP address to prevent bypass via refresh
      const identifier = socket.handshake.address || socket.id;
      const { allowed, remainingSeconds } = await checkAndSetRequestTimer(roomCode, identifier, cooldownSeconds);
      ...
      await addTrackToQueue(roomCode, safeTrack);
      await voteTrack(roomCode, safeTrack.id, 1); // Auto-upvote new requests
  ...
  socket.on('vote', async ({ roomCode, trackId, increment }) => {
    try {
      // Force increment to exactly 1 to prevent manipulation
      const safeIncrement = 1;
      await voteTrack(roomCode, String(trackId), safeIncrement);
      const queue = await getQueue(roomCode);
